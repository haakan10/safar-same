...
<body>
    ...
    if ($loggedIn && $_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST["submit"])) {
            ...
            $to_Cities = $_POST["to_Cities"];
            $cost = $_POST["stepCost"];
            $deptime = $_POST["dep_time"];
            $depdate = $_POST["dep_date"];
            $busno = $_POST["busno"];
            $route_exists = exist_routes($conn, $viaCities,$to_Cities, $depdate, $deptime,);
            $route_added = false;
            

            if (!$route_exists) {
                // Route is unique, proceed
                $sql = "INSERT INTO `routes` (`route_cities`, `to_cities`, 
                     `bus_no`, 
                     `route_dep_date`,
                     `route_dep_time`, `route_step_cost`, `route_created`) VALUES ('$viaCities', '$to_Cities','$busno', '$depdate','$deptime', '$cost', current_timestamp());";
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="addRouteForm" action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="POST">
                        <div class="mb-3">
                            <label for="viaCities" class="form-label">From Cities</label>
                            <input type="text" class="form-control" id="viaCities" name="viaCities" placeholder="from city" required>
                            <span id="error">

                            </span>
                        </div>
                        <div class="mb-3">
                            <label for="to_Cities" class="form-label">To Cities</label>
                            <input type="text" class="form-control" id="to_Cities" name="to_Cities" placeholder="to city" required>
                            <span id="error">
